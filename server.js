...
const preferredPorts = [
  Number.isNaN(Number(process.env.PORT)) ? 3000 : Number(process.env.PORT),
  3001,
  3002,
];
...
function startServer() {
  const port = preferredPorts[currentPortIndex];
  ...
  server.on("error", (error) => {
    if (error.code === "EADDRINUSE") {
      if (currentPortIndex + 1 < preferredPorts.length) {
        console.warn(
          `Port ${port} is already in use. Trying port ${preferredPorts[currentPortIndex + 1]}...`,
        );
        currentPortIndex += 1;
        startServer();
      } else {
        console.error(
          `Ports ${preferredPorts.join(", ")} are all in use. Stop the other processes or set a different PORT before restarting.`,
        );
        process.exit(1);
      }
    } else {
